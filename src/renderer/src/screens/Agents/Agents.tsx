...
function Agents({
  ...
  const loadProfiles = useCallback(async (): Promise<void> => {
    const list = await desktopClient.listProfiles();
    setProfiles(list);
    setLoading(false);
  }, []);
  ...
  async function handleCreate(): Promise<void> {
    ...
    setCreating(true);
    setError("");
    const result = await desktopClient.createProfile(name, cloneConfig);
    setCreating(false);
    if (result.success) {
      setShowCreate(false);
      setNewName("");
  ...
  async function handleDelete(name: string): Promise<void> {
    const result = await desktopClient.deleteProfile(name);
    if (result.success) {
  ...
  async function handleSelect(name: string): Promise<void> {
    await desktopClient.setActiveProfile(name);
