...
function Schedules({ profile }: SchedulesProps): React.JSX.Element {
  ...
  const loadJobs = useCallback(async (): Promise<void> => {
    try {
      const list = await desktopClient.listCronJobs(true, profile);
      setJobs(list);
    } catch {
  ...
  async function handleCreate(): Promise<void> {
    ...
    setError("");
    try {
      const result = await desktopClient.createCronJob(
        buildSchedule(),
  ...
  async function handleRemove(jobId: string): Promise<void> {
    ...
    setError("");
    try {
      const result = await desktopClient.removeCronJob(jobId, profile);
  ...
  async function handleToggle(job: CronJob): Promise<void> {
    ...
    try {
      const result =
        job.state === "paused"
          ? await desktopClient.resumeCronJob(job.id, profile)
          : await desktopClient.pauseCronJob(job.id, profile);
      if (result.success) {
        await loadJobs();
      } else {
  ...
  async function handleTrigger(jobId: string): Promise<void> {
    ...
    setError("");
    try {
      const result = await desktopClient.triggerCronJob(jobId, profile);
      if (result.success) {
        await loadJobs();
      } else {
