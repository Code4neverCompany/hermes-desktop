...
function Sessions({
  ...
  const loadSessions = useCallback(async (): Promise<void> => {
    setLoading(true);
    const cached = await desktopClient.listCachedSessions(50);
    if (cached.length > 0) {
      setSessions(cached);
      setLoading(false);
    }
    const synced = await desktopClient.syncSessionCache();
  ...
  useEffect(() => {
    ...
    searchTimer.current = setTimeout(async () => {
      const results = await desktopClient.searchSessions(searchQuery);
