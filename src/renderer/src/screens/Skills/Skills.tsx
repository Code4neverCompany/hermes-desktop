...
function Skills({ profile }: SkillsProps): React.JSX.Element {
  ...
  const loadInstalled = useCallback(async (): Promise<void> => {
    const list = await desktopClient.listInstalledSkills(profile);
    setInstalledSkills(list);
  }, [profile]);

  const loadBundled = useCallback(async (): Promise<void> => {
    const list = await desktopClient.listBundledSkills();
  ...
  async function handleViewDetail(skill: InstalledSkill): Promise<void> {
    ...
    const content = await desktopClient.getSkillContent(skill.path);
  ...
  async function handleInstall(name: string): Promise<void> {
    ...
    setError("");
    const result = await desktopClient.installSkill(name, profile);
  ...
  async function handleUninstall(name: string): Promise<void> {
    setActionInProgress(name);
    setError("");
    const result = await desktopClient.uninstallSkill(name, profile);
    setActionInProgress(null);
    if (result.success) {
      setDetailSkill(null);
      await loadInstalled();
